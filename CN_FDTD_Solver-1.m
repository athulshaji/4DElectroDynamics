...
tic
Dxew1 = Dxe.*(a2./WX);Dxew2=Dxew1;
% Dxxew = (Dxxe(i,:)./(WX.'));
Dxhw1 = Dxh.*(a1./WX);Dxhw2=Dxhw1;

Dyew1 = Dye.*(a2./WY);Dyew2=Dyew1;
% Dyyew = (Dyye(i,:)./(WY.'));
Dyhw1 = Dyh.*(a1./WY);Dyhw2=Dyhw1;

Dzew1 = Dze.*(a2./WZ);Dzew2=Dzew1;
% Dzzew = (Dzze(i,:)./(WZ.'));
Dzhw1 = Dzh.*(a1./WZ);Dzhw2=Dzhw1;
toc
...
SOURCE_VOLTAGE_WALL=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);    
for yloc=YRANGE %28:32
    for xloc=XRANGE
        for zloc=ZRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;      
            %A(0*Nxyz+xyzloc,0*Nxyz+xyzloc)=0;Dzhw1(xyzloc,:)=0;Dyhw2(xyzloc,:)=0;                      
            %A(1*Nxyz+xyzloc,1*Nxyz+xyzloc)=0;Dxhw1(xyzloc,:)=0;Dzhw2(xyzloc,:)=0;
            %A(2*Nxyz+xyzloc,2*Nxyz+xyzloc)=1;Dyhw1(xyzloc,:)=0;Dxhw2(xyzloc,:)=0;  
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
END_VOLTAGE_WALL=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);   
for yloc=YRANGE %28:32
    for xloc=XRANGE
        for zloc=ZRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;      
...
VOLTAGE_WALL_1=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);     
for yloc=YRANGE %28:32
    for xloc=XRANGE
        for zloc=ZRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;      
...
VOLTAGE_WALL_2=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);      
for yloc=YRANGE %28:32
    for xloc=XRANGE
        for zloc=ZRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;      
...
CURRENT_LINE_21=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);
%%%%%%%%%%%%%%%%%%%%%%%
% 
% 
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE_22=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);
%%%%%%%%%%%%%%%%%%%%%%%
% 
% 
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE_23=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);
%%%%%%%%%%%%%%%%%%%%%%%
% 
% 
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE_24=ThreeDimNodeLoc2VectorNodesLoc(XRANGE,YRANGE,ZRANGE,Nxy,Ny);
%%%%%%%%%%%%%%%%%%%%%%%
% 
% 
...
% YRANGE=76;XRANGE=64:68;ZRANGE=39;
...
% YRANGE=76;XRANGE=64:68;ZRANGE=37;
...
for n = 1:T
    ...
    EzNew(SOURCE_VOLTAGE_WALL)=INPUT_AMP*sin(2*pi*(FREQ)*(((n-1)*delt)));  
    %ExNew(SOURCE_VOLTAGE_WALL)=0; EyNew(SOURCE_VOLTAGE_WALL)=0;  

    time=(n-1)*delt;
    CIRCUIT_CURRENT_INPUT(n)=((sum((HxNew(CURRENT_LINE_11)))*delx)-(sum((HxNew(CURRENT_LINE_12)))*delx)+(sum((HzNew(CURRENT_LINE_13)))*delz)-(sum((HzNew(CURRENT_LINE_14)))*delz));
   %     I1=(HxNew(CURRENT_LINE_11)+HxNew(CURRENT_LINE_111))/2;
   %     I2=(HxNew(CURRENT_LINE_12)+HxNew(CURRENT_LINE_122))/2;
   %     I3=(HxNew(CURRENT_LINE_13)+HxNew(CURRENT_LINE_133))/2;
   %     I4=(HxNew(CURRENT_LINE_14)+HxNew(CURRENT_LINE_144))/2;
   %
   %     CIRCUIT_CURRENT_INPUT(n)=sum(I1)*delx-sum(I2)*delx+sum(I3)*delz-sum(I4)*delz;
    ...
    EzNew(VOLTAGE_WALL_1)=((V0(n))/(delz*2)); %ExNew(VOLTAGE_WALL_1)=0; EyNew(VOLTAGE_WALL_1)=0;
    EzNew(VOLTAGE_WALL_2)=(V2(n)/(delz*2));% ExNew(VOLTAGE_WALL_2)=0; EyNew(VOLTAGE_WALL_2)=0;

    %EzNew(PEC_WALL1)=0;  ExNew(PEC_WALL1)=0; EyNew(PEC_WALL1)=0;   

    EzNew(END_VOLTAGE_WALL)=Z0*((sum(HxNew(CURRENT_LINE_21))*delx)-(sum(HxNew(CURRENT_LINE_22))*delx)+(sum(HzNew(CURRENT_LINE_23))*delz)-(sum(HzNew(CURRENT_LINE_24))*delz));
    ...
    if(PML_ENABLED)
    psiEXY = VY.*psiEXY + Dyhw1*((HzNew + HzOld));
    psiEXZ = VZ.*psiEXZ + Dzhw1*((HyNew + HyOld));
    psiEYZ = VZ.*psiEYZ + Dzhw1*((HxNew + HxOld));
    psiEYX = VX.*psiEYX + Dxhw1*((HzNew + HzOld));
    psiEZX = VX.*psiEZX + Dxhw1*((HyNew + HyOld));
    psiEZY = VY.*psiEZY + Dyhw1*((HxNew + HxOld));

    psiHXY = VY.*psiHXY + Dyew1*((EzNew + EzOld));
    psiHXZ = VZ.*psiHXZ + Dzew1*((EyNew + EyOld));
    psiHYZ = VZ.*psiHYZ + Dzew1*((ExNew + ExOld));
    psiHYX = VX.*psiHYX + Dxew1*((EzNew + EzOld));
    psiHZX = VX.*psiHZX + Dxew1*((EyNew + EyOld));
    psiHZY = VY.*psiHZY + Dyew1*((ExNew + ExOld));      
...
field_probe_1(n)=FIELDEZ(66,36,37);field_probe_2(n)=FIELDHX(66,36,39)*200;
%figure(2),grid on;plot(field_probe_1);
%figure(3),grid on;plot(field_probe_2);
    if(VIDEO && n>3)        
        val = log10((INPUT_AMP));
        %subplot 121 %234
...
% function [MNA,RHS,LHS,Rload,Rser,Cload,Csrc]=SPICE_INIT_Rser_RC_NW(delt)
% 
% MNA=zeros(4);
% 
% % load
...
% end
% 
% function [MNA,RHS,LHS]=MySPICE2(MNA,RHS,LHS,V1,V2,SRC,delt,delt_old,Rload,Rser,Cload,Csrc)
% Vnode20pre=LHS(2);
% Vnode10pre=LHS(1);
% for n=time:delt:time+(delt_old-delt)
%     RHS=[SRC;...
