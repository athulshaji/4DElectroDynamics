...
% % PEC WALL1
% PEC_WALL1=[];
% ZRANGE=32:34;YRANGE=120;XRANGE=88:94;
% for zloc=ZRANGE
%     for yloc=YRANGE
%         for xloc=XRANGE
%             xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
%             PEC_WALL1=[PEC_WALL1;xyzloc];
%             Dyhw(xyzloc,:)=0;
%             Dxhw(xyzloc,:)=0;
%             Dzhw(xyzloc,:)=0;
%             A(2*Nxyz+xyzloc,2*Nxyz+xyzloc)=0;
%             A(0*Nxyz+xyzloc,0*Nxyz+xyzloc)=0;
%             A(1*Nxyz+xyzloc,1*Nxyz+xyzloc)=0;
%         end
%     end
...
for n = 1:T
    ...
    EzNew(SOURCE_WALL)=(INPUT_AMP*S)*sin(2*pi*(FREQ)*(((n-1)*delt)))/(2*delz);
    ExNew(SOURCE_WALL)=0; EyNew(SOURCE_WALL)=0;

    %     EzNew(END_VOLTAGE_WALL)=(Z0*((sum(HxNew(CURRENT_LINE21))*delx)-(sum(HxNew(CURRENT_LINE22))*delx)+(sum(HzNew(CURRENT_LINE23))*delz)-(sum(HzNew(CURRENT_LINE24))*delz)))/(2*delz);
    %     ExNew(END_VOLTAGE_WALL)=0; EyNew(END_VOLTAGE_WALL)=0;
   
    %     REC_VOLTAGE_SRC_END(n)=max(EzNew(SOURCE_WALL));
    %     REC_VOLTAGE_LOAD_END(n)=max(EzNew(END_VOLTAGE_WALL));

    if(CIRCUIT)
        time=(n-1)*delt;
        CIRCUIT_CURRENT_INPUT(n)=((sum((HxNew(CURRENT_LINE11)))*delx)-(sum((HxNew(CURRENT_LINE12)))*delx)+(sum((HzNew(CURRENT_LINE13)))*delz)-(sum((HzNew(CURRENT_LINE14)))*delz));
        %CIRCUIT_CURRENT_INPUT(n)=((sum((HxNew(CURRENT_LINE21)))*delx)-(sum((HxNew(CURRENT_LINE22)))*delx)+(sum((HzNew(CURRENT_LINE23)))*delz)-(sum((HzNew(CURRENT_LINE24)))*delz));
        ...
        %EzNew(END_VOLTAGE_WALL)=V0(n)/(delz*2);
        %ExNew(END_VOLTAGE_WALL)=0; EyNew(END_VOLTAGE_WALL)=0;
    end
