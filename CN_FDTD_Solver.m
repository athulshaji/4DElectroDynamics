...
%% Initializing MATLAB
Pin_dBm=-20;
Z0=sqrt((4*pi*1e-7*1)/(8.854e-12*4.4));
INPUT_AMP=My_dBm2Watt(Pin_dBm,Z0);
FREQ = fmax;
VIDEO = 0; % numerals 0 or 1
PML_ENABLED = 1;
CIRCUIT=1;
file_name = '15July2022-pure-resistive-load.avi';
%% Basic parameter
...
%%%%%%%%%%%%%%%%%%%%%%%
% SOURCE WALL
YRANGE=36;XRANGE=65:67;ZRANGE=37;
SOURCE_WALL=[];
...
% VOLTAGE WALL1
VOLTAGE_WALL1=[];
YRANGE=76;XRANGE=65:67;ZRANGE=37;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
...
% CURRENT 1
CURRENT_LINE11=[];
%YRANGE=114:115;XRANGE=92:100;ZRANGE=40;
YRANGE=75;XRANGE=64:68;ZRANGE=39;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE11=[CURRENT_LINE11;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
% CURRENT 1
CURRENT_LINE12=[];
%YRANGE=114:115;XRANGE=92:100;ZRANGE=39;
YRANGE=75;XRANGE=64:68;ZRANGE=37;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE12=[CURRENT_LINE12;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
% CURRENT 1
CURRENT_LINE13=[];
%YRANGE=114:115;XRANGE=92;ZRANGE=39:40;
YRANGE=75;XRANGE=64;ZRANGE=37:39;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE13=[CURRENT_LINE13;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
% CURRENT 1
CURRENT_LINE14=[];
%YRANGE=114:115;XRANGE=100;ZRANGE=39:40;
YRANGE=75;XRANGE=68;ZRANGE=37:39;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE14=[CURRENT_LINE14;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE31=[];
%YRANGE=114:115;XRANGE=92:100;ZRANGE=40;
YRANGE=86;XRANGE=64:68;ZRANGE=39;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE31=[CURRENT_LINE31;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE32=[];
%YRANGE=114:115;XRANGE=92:100;ZRANGE=39;
YRANGE=86;XRANGE=64:68;ZRANGE=37;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE32=[CURRENT_LINE32;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE33=[];
%YRANGE=114:115;XRANGE=92;ZRANGE=39:40;
YRANGE=86;XRANGE=64;ZRANGE=37:39;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE33=[CURRENT_LINE33;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
CURRENT_LINE34=[];
%YRANGE=114:115;XRANGE=100;ZRANGE=39:40;
YRANGE=86;XRANGE=68;ZRANGE=37:39;
for zloc=ZRANGE
    for yloc=YRANGE
        for xloc=XRANGE
            xyzloc=Nxy*(zloc-1)+Ny*(xloc-1)+yloc;
            CURRENT_LINE34=[CURRENT_LINE34;xyzloc];
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
IC=[0;0;0;0;];
Ickt=0;index=0;V0=[];V1=[];V2=[];V3=[];I_branch=0;V2pre=0;
Vnode12pre=0;
Vnode34pre=0;
Vnode40pre=0;
ILmatchpre=0;
...
for n = 1:T
    ...
    if(PML_ENABLED)
        ...
        RHS_STAR = (A+D1+(2*D2))*PHI_OLD + (INC_FIELD);
        PHI_STAR = U_STAR \ (L_STAR \ (P_STAR * RHS_STAR));
        %[PHI_STAR,flag1]=gmres(EYE-D1,RHS_STAR,10,[],size(RHS_STAR,1),L_STAR,U_STAR);
        RHS = PHI_STAR - (D2*(PHI_OLD));
        PHI_NEW = U \ (L \ (P * (RHS)));
    ...
    EzNew(SOURCE_WALL)=((S*INPUT_AMP)/(2*delz))*(sin(2*pi*FREQ*((n-1)*delt)));
    %EzNew(SOURCE_WALL)=((INPUT_AMP)/(2*delz))*(exp(-(((n-1)*delt-t0)/(tau)).^2))*(sin(2*pi*FREQ*((n-1)*delt)));
    %EzNew(SOURCE_WALL)=((INPUT_AMP)/(2*delz))*(exp(-(((n-1)*delt-t0)/(tau)).^2));
    ExNew(SOURCE_WALL)=0; EyNew(SOURCE_WALL)=0;
   SRC(n)=((INPUT_AMP)/(2*delz))*(sin(2*pi*FREQ*((n-1)*delt)));    

    if(CIRCUIT)
        time=(n-1)*delt;
        CIRCUIT_CURRENT_INPUT_1(n)=((sum((HxNew(CURRENT_LINE11)))*delx)-(sum((HxNew(CURRENT_LINE12)))*delx)+(sum((HzNew(CURRENT_LINE13)))*delz)-(sum((HzNew(CURRENT_LINE14)))*delz));
        CIRCUIT_CURRENT_INPUT_2(n)=((sum((HxNew(CURRENT_LINE31)))*delx)-(sum((HxNew(CURRENT_LINE32)))*delx)+(sum((HzNew(CURRENT_LINE33)))*delz)-(sum((HzNew(CURRENT_LINE34)))*delz));
        input_source=[  CIRCUIT_CURRENT_INPUT_1(n);...
                        CIRCUIT_CURRENT_INPUT_2(n);];
        [MNAckt,RHSckt,LHSckt,LHScktpre,V0,V1,V2,V3,G0,I0,Vnode12pre,Vnode40pre,ILmatchpre,Vdiode,Vdiodepre,tol_value,index,I_branch,diode_current,src_current,vd,vd2,I]=MySPICE(MNAckt,RHSckt,LHSckt,LHScktpre,Cdiode,Cload,Rload,Cmatch,Lmatch,Csrc,V0,V1,V2,V3,G0,I0,tol_value,input_source,time,delt_ckt,delt,index,Vnode12pre,Vnode40pre,ILmatchpre,Vdiode,Vdiodepre);
        Vd(n)=vd;Idiode(n)=vd/Rs;
        Vd2(n)=vd2;Idiode2(n)=vd2/Rs;
        EzNew(VOLTAGE_WALL1)=((V0(n))/(delz*2)); ExNew(VOLTAGE_WALL1)=0; EyNew(VOLTAGE_WALL1)=0;
        EzNew(VOLTAGE_WALL2)=(V3(n)/(delz*2)); ExNew(VOLTAGE_WALL2)=0; EyNew(VOLTAGE_WALL2)=0;
        ...
        CIRCUIT_CURRENT_INPUT_3(n)=((sum(HxNew(CURRENT_LINE21))*delx)-(sum(HxNew(CURRENT_LINE22))*delx)+(sum(HzNew(CURRENT_LINE23))*delz)-(sum(HzNew(CURRENT_LINE24))*delz));
        LHS2=MySPICE2(CIRCUIT_CURRENT_INPUT_3(n),LHS2,delt);
        VOLTAGE_END_PORT(n)=LHS2(1);
        EzNew(END_VOLTAGE_WALL)=VOLTAGE_END_PORT(n)/(2*delz);
    ...
    if(VIDEO && n>3)
    field_probe=FIELDEZ(66,36:95,37);
    grid on;plot(field_probe);ylim([-2.1*INPUT_AMP/(2*delz),2.1*INPUT_AMP/(2*delz)]);title(['iter# ',num2str(n)]);hold on;
    field_probe=FIELDHZ(68,36:95,38);
    grid on;plot(field_probe*Z0);ylim([-2.1*INPUT_AMP/(2*delz),2.1*INPUT_AMP/(2*delz)]);title(['iter# ',num2str(n)]);
...
My_MATLAB_Conversion_Efficiency(Vin,Iin,Vout,Iout,delt,FREQ)
figure,hold on;grid on;plot(0:delt:(n-1)*delt,V1,'r');plot(0:delt:(n-1)*delt,V3,'k');drawnow;
end
function [Vpeak]=My_dBm2Watt(dBm_val,Z0)
clc;close all;
format long
watt_val=10^((dBm_val/10)-3);
vrms=sqrt(2*Z0*watt_val);
Vpeak=vrms*sqrt(2);
end
...
function [MNA,RHS,LHS,LHSpre,V0,V1,V2,V3,G0,I0,Vnode12pre,Vnode40pre,ILmatchpre,Vdiode,Vdiodepre,tol_value,index,I_branch,diode_current,src_current,vd,vd2,I]=MySPICE(MNA,RHS,LHS,LHSpre,Cdiode,Cload,Rload,Cmatch,Lmatch,Csrc,V0,V1,V2,V3,G0,I0,tol_value,input_source,time,delt,delt_old,index,Vnode12pre,Vnode40pre,ILmatchpre,Vdiode,Vdiodepre)
Rs=20;
Isat=5e-6; % 5uA
Vth=25.85e-3; % 25.85 mV @ 300K
Vdiode=0;
...
function [LHS2]=MySPICE2(I,LHS2,delt)
Vnode10pre=LHS2(1);
Csrc=2e-14;
MNA(1,1)=1/50;
MNA(2,1)=Csrc/delt;
