...
while(~feof(fid))
    ...
    m{1,1}=str2num(m{1,1});
    m{1,2}=str2num(m{1,2});
    Diode_1N4148.voltage(counter)=m{1};
    Diode_1N4148.current(counter)=m{2};
    counter=counter+1;
end
fclose(fid);
...
if(REAL_DIODE_DATA)
    ...
    for time=1:length(vdiode)
        if (vdiode(time)>0)
            inonlinear_node1(time)=interp1(real(Diode_1N4148.voltage),...
                real(Diode_1N4148.current),real(vdiode(time)),'spline');
...
%     real_inonlinear_node1=interp1(real(Diode_1N4148.voltage),real(Diode_1N4148.current),real(vdiode),'spline');
%     % imag_inonlinear_node1=interp1((Diode_1N4148.voltage),(Diode_1N4148.current),imag(vdiode),'spline');
...
percentage_RF2DC_conversion_efficiency=abs((real((VNONLINEAR_NODE2_VEC(K+1))*conj(ILINEAR_NODE2_VEC(K+1))))/...
...
%     % inonlinear_node1=interp1(Diode_1N4148.voltage,Diode_1N4148.current,vdiode,'spline');
%     real_inonlinear_node1=interp1((Diode_1N4148.voltage),(Diode_1N4148.current),real(vdiode),'spline');
%     % imag_inonlinear_node1=interp1((Diode_1N4148.voltage),(Diode_1N4148.current),imag(vdiode),'spline');
%     inonlinear_node1=real_inonlinear_node1;%+1i*imag_inonlinear_node1; 
end
if(DIODE_EQN_BASED)
...
if(REAL_DIODE_DATA)
    ...
    for time=1:length(vdiode)
        if (vdiode(time)>0)
            inonlinear_node2(time)=interp1(real(Diode_1N4148.voltage),...
                real(Diode_1N4148.current),real(vdiode(time)),'spline');
        elseif(vdiode(time)<0 && vdiode(time)>=Bv)
...
%     % inonlinear_node2=interp1(Diode_1N4148.voltage,Diode_1N4148.current,vdiode,'spline');
%     real_inonlinear_node2=interp1(real(Diode_1N4148.voltage),real(Diode_1N4148.current),real(vdiode),'spline');
%     % imag_inonlinear_node2=interp1((Diode_1N4148.voltage),(Diode_1N4148.current),imag(vdiode),'spline');
%     inonlinear_node2=real_inonlinear_node2;%+1i*imag_inonlinear_node2;
end
if(DIODE_EQN_BASED)
    vdiode=real(vnonlinear_node1-vnonlinear_node2);
    for time=1:length(vdiode)
