function MyFunctionSPICE_diode_series_RC_nw_ver2
...
for time=start_time:delt:end_time   
    ...
    while(tol>tol_value)
        ...
        I0=Idiodepre-G0*(Vdiodepre-LHS(3)+LHS(4));         
        % junction capacitance is evaluated             
        if Vdiodepre<=Fc*Vj
            Cj=AREA*Cj0*(1-(Vdiodepre-LHS(3)+LHS(4))/Vj).^-M;
        elseif Vdiodepre>Fc*Vj
            Cj=AREA*(Cj0/(1-Fc)^M)*(1+(M/(Vj*(1-Fc)))*((Vdiodepre-LHS(3)+LHS(4))-Fc*Vj));
        end
...
toc;
figure(1),plot(0:delt:end_time,V1);grid on;hold on;figure(2),plot(0:delt:end_time,V2/2);grid on;hold on;
