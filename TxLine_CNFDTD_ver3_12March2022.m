function TxLine_CNFDTD_ver3_12March2022
...
AMP=11.2468; %dbmtov(Pin,Zin);Pin=5dBm;Zin=50ohms
t=0:Tsteps-1;
...
%     Ickt=raw_data_3.variable_mat(3,end);
    ...
    Ickt=raw_data_3.variable_mat(5,end);
    Ipre=VI(2*Mz);
    Idiode(n)=raw_data_3.variable_mat(6,end);
    Vleft(n)=raw_data_3.variable_mat(2,end);
    Vright(n)=raw_data_3.variable_mat(3,end);
    Vrecord(n)=VI(ceil(Mz-1));    
    Vi(n)=VI(2);
    Ii(n)=VI(Mz+2);
    %     plot(VI(1:Mz));
    %     ylim([-2,2]);grid on;
    %     drawnow;
    if(mod(n,1000)==0)
        save 16march2022\case-S=1-Pin=25dBm-11.2468Vp.mat
    end
